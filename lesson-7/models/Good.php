...
class Good {
    ...
    public static function getOrders() {
        ...
        $orders = DB::connect()->select("SELECT orders.id, orders.time, orders.good_id, goods.title, goods.price FROM orders LEFT JOIN goods ON orders.good_id = goods.id WHERE user_id = $id ORDER BY time DESC");
        return $orders;
    }
    ...
    public static function getProduct() {
        return DB::connect()->select("SELECT * FROM goods WHERE id = {$_GET['id']}")[0];
    }
}